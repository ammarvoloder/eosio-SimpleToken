...
void token::create()
{
    ...
    auto sym = symbol("STK", 4);
    auto maximum_supply = asset(10000000, sym);
...
void token::issue(const asset& quantity, const string& memo )
{
    ...
    auto stksym_code = symbol("STK", 4);
    check(sym.code() == stksym_code.code(), "This contract can handle STK tokens only." );
    check(sym.is_valid(), "invalid symbol name");
    ...
    check( quantity.amount <= existing_token.max_supply.amount - existing_token.supply.amount, "quantity exceeds available supply");
                      ...
                      const string&  memo )
{
    ...
    auto stksym_code = symbol("STK", 4);
    check(sym == stksym_code.code(), "This contract can handle STK tokens only.");
